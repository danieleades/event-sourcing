...
Snapshots are opt-in. If you enable snapshots (via `Repository::with_snapshots`), the aggregate
state must be serializable so it can be persisted and restored:

- `Account: serde::Serialize`
- `Account: serde::de::DeserializeOwned`
